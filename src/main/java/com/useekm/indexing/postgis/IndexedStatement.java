...
public class IndexedStatement {
	...
	public static PGgeometry asGeometry(Literal literal, boolean acceptNoType)
			throws IndexException {
...
//			org.postgis.Geometry geometry = result.getGeometry();
	...
	public static PreparedStatement addPrepare(Connection conn, String tableName)
			throws SQLException {
		return conn.prepareStatement(INSERT_INTO + tableName + INSERT_VALUES);
	}

	public static void addNewBatch(PreparedStatement stat,
			IndexedStatement indexedStatement) throws SQLException {
		int idx = 1;
